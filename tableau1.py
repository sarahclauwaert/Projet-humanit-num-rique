...
if len(tables) >= 2:
    table = tables[1]  # Accès au deuxième tableau
else:
    print("Il n'y a pas de deuxième tableau sur la page.")
    ...
    if len(cells) == 1:
        if prev_row_data:  # Vérifier s'il y a des données dans la ligne précédente
            for header, value in prev_row_data.items():
                if header == "Communes supprimées":
                    nouvelle_commune_supprimee = cells[0].text.strip()
                    if nouvelle_commune_supprimee not in value:
                        prev_row_data[header] = value + ", " + nouvelle_commune_supprimee  # Concaténer les valeurs
                    else:
...
with open('data_table_1.json', 'w', encoding='utf-8') as f:
